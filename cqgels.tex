...
supercoiling of endogenous plasmids of \raim{\textit{Synechococcus
...
\cite{Jones1980}. At this lower affinity of the intercalator
topoisomer separation in agarose gels `\textit{less sensitive to
  variations in experimental conditions}' \cite{Shure1977}.
Additionally, intercalator effects on DNA supercoiling are potentially
sequence dependent \cite{Vetcher2010}.
%
...
\raim{ \citet{Mitchenall2018} reported separation of topoisomers of
  pBR322, pUC19, and a 339 bp DNA minicircle on a commercial capillary
  gel electrophoresis platform (Qiagen QIAxcel Advance System), where
  EtBr is part of the gel delivered in a cartridge. The system is
  quite sensitive and samples of \SI{100}{\ng} DNA sufficed. Moreover,
  the range of separated topoisomers was broader than on agarose
  gels. This would allow to use less runs with intercalator
  concentrations to achieve full separation of \textit{in vivo} levels
  of supercoiling.  However, the intercalator type and concentration
  can not be manipulated on this system. We recently succeeded to
  separate pUC19 topoisomers (XYZ ng/uL) by adding comparatively large
  ...
  by Agilent). But the results were not consistent between runs. More
  work and alternatives to the existing commercial systems is required
  to establish topoisomer separation by capillary gel electrophoresis
  as a standard procedure.  And finally, the analysis of
  electropherograms is also a limiting factor. No standards or
  established tools exist on how to calculate average linking number
  differences between samples. To this end \citet{Ziraldo2019} provide
  a plug-in for the ImageJ image analysis software that should
  streamline the analysis of agarose gels. Here, we also used ImageJ
  to extract electropherograms from gel images, and peak detection
  routines from a mass-spectrometry R package, \texttt{msProcess} (see
  Methods) which is not maintained anymore, to calculate average
  linking numbers in R. A novel package, \texttt{bioanalyzeR}
  (\url{https://github.com/jwfoley/bioanalyzeR}), allows to parse
  electropherograms from data files exported by Agilent capillary
  electrophoresis platforms (Bioanalyzer, TapeStation). Automated
  analysis tools for both gel images and capillary electrophoresis
  platforms could open this old and elegant method for modern
