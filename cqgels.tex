...
\cite{Smith2006}. Woelfle \etal{} \cite{Woelfle2007} then used
an old method, separation of plasmid topoisomers by agarose gel
electrophoresis, to show that the supercoiling level of its
...
contribution and requires some background.
Textbook plasmid agarose gels show three distinct bands: the
``linear'' form which underwent a double strand break, likely during
extraction; and the \texttt{oc } (open circular) band, which has only
one or more single strand breaks and thus is still circular. The
latter travels slowest on the gel. And thirdly, intact plasmids travel
fastest as a smeared band, widely known as the \texttt{ccc} band, for
the covalently closed circular form of the plasmid. The band is
smeared since it actually contains several distinct species, denoted
...
$\dlkr=\lk-\lkr$.  Negatively supercoiled DNA has $\dlk<0$ and
positively supercoiled DNA has $\dlkr>0$, or as the supercoiling
density $\sigma=\frac{\dlkr}{\lkr}$.  Plasmids isolated from
...
faster.
%
Choosing more neutral band identifiers based on the gel images in
the two publications makes these differences clearer:

\begin{align}
  RM &= \frac{target - upper}{lower - upper}\\
  RM &=1-\frac{target - upper}{\lvert upper - lower \rvert}
\end{align}

This different interpretation of the gels of \cite{Woelfle2007} would
imply that negatively supercoiling rapidly increases upon onset of the
light phase and would progressively decrease in prolonged darkness.
Vijayan \etal{} measured supercoiling only in constant
light conditions, and did not mention or discuss this different
interpretations. Thus, the DNA supercoiling dynamics within the natural
light/dark cycle remains unclear.
...
\SI{0.06}{\ugml}. 
%That is, in the topoI reaction, up to 45x more EtBr
%was used in the topoisomerase I relaxtion reactions than in the gels
%to achieve the similar plasmid relaxation effects.
%
%lanes 11 and 9 have in vivo supercoiling
%
Full relaxation of the SV40 DNA was achieved at
$\cqgel=\SI{0.06}{\ugml}$ in the gel buffer () and at
$\cqtop\approx\SI{1.7}{\ugml}$ (lane 9 Figures 1 and 3 of
\cite{Keller1975b}: ca. $\SI{4.5}{\uM}$) in the topoI reaction buffer
().
%
%That is, the CQ concentration required to fully quench all
%supercoiling was $\approx$30 lower in the
%gel buffer than in the topoI reaction buffer.
...
%
Thus, the concentration to fully relax the
original level of DNA supercoiling was \SIrange{1}{4}{\ugml} in the
agarose gel buffer (0.5x TBE), and \SIrange{1000}{3000}{\ugml} in the CutSmart
reaction buffer.
%
%That is, the CQ concentration required to fully quench all
%supercoiling of pUC19 in \textit{E. coli} was $\approx$1000x lower in
%the gel buffer than in the reaction buffer.

\paragraph{Discussion: CQ vs EtBr.}
In summary, the intercalator concentration required to fully quench
the original level supercoiling of SV40 and pUC19 was $\approx$30x and
...
higher during the light or dark phases, we measured supercoiling levels
